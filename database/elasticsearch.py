...
class Elastic(Elasticsearch):
    def __init__(self, host, username: str, password:str) -> None:
        self.es = Elasticsearch(hosts=host, http_auth=(username, password), verify_certs=False)
        if self.es.ping():
    ...
    def add(self, index, data, id=None):
        ...
        try:
            response = self.es.index(index=index, id=id, document=data)
    ...
    def find(self, index, id):
        try:
            response = self.es.get(index=index, id= id)
